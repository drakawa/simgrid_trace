...
//  chaix
  if(XBT_LOG_ISENABLED(chaix,	xbt_log_priority_verbose )){
	  ...
	  // 	  strcat(routeStr,"&");
	  // }
	  XBT_CVERB(chaix,"%s,%s,%g,%g,%g,%g,%s", src->getName().c_str(), dst->getName().c_str(), size, action->rate_, SIMIX_get_clock(), action->latency_, routeStr.c_str());
  }
